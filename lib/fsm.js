...
function Node(transitions, partials, final){
	...
	this.classes = Object.keys(transitions).filter(v => v.length>1);
}
...
function sort_final(a, b){
	const cmp = compare([a.route.fsm, b.route.fsm]);
	if(cmp[0]===true && cmp[1]===true) return 0;
	if(cmp[0]===true && cmp[1]===false) return 1;
	if(cmp[0]===false && cmp[1]===true) return -1;
	if(cmp[0]===false && cmp[1]===false) return 0;
}
...
function union(fsms){
	...
	function final_union(states){
		if(states.some(final => final && (Array.isArray(final) ? final.length : final))){
			...
			if(items.sort) items.sort(sort_final);
...
function compare(fsms){
	...
	function final_union(states){
		...
		if(!!a && !!b) isDisjoint = false; // are there not any elements in common
	}
